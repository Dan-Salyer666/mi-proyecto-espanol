...
- **Always inherited (site-wide, non-negotiable):** the design *language* — the root CSS tokens, DM Serif Display + Outfit, the English **NOTA\*** layer on Spanish-primary pages, neutral-voice TTS, grading colors, back-nav, the site favicon, and a chosen background. These apply at every size.
...
### Favicon
Every page, at any directory depth, carries the site favicon (`/assets/bandera/`). Paths are **absolute** — the identical block works in root, `/topics/`, `/interactive/`, and `/topics/subpages/`. Place immediately after `</title>`.
```html
...
```
- Never convert to relative paths.
- A page without the favicon block is not done.
### Cards / Buttons / Nav
...
- **Surgical edits over full rewrites** — targeted changes applied in VS Code, never full-file regeneration of a working file.
- **Offer before handing off** — for any small/surgical edit, Claude first **asks Daniel whether he wants to apply it himself** (in the format below) **or have Claude apply it into the working file and hand the file back**. Default to asking unless Daniel has already stated a preference for the task at hand. (Daniel is building coding confidence — the handoff format has to be foolproof.)
- **Surgical-edit handoff format** — when Daniel applies an edit himself, Claude gives, *per edit*:
  1. the **file** (full path) and a one-line note on where in it;
  2. a **FIND block** — a fenced code block with the **exact, verbatim** existing code to locate; include enough surrounding lines that the snippet is **unique** (appears exactly once in the file);
  3. a **REPLACE block** — a fenced code block showing that **same region after the change** (the anchor lines plus the inserted/changed code), so Daniel selects the FIND text in VS Code and pastes REPLACE over it, then eyeballs the result.
  One edit = one FIND/REPLACE pair. Never "add this near X" without an anchor. Line numbers are **hints only** — the anchor text is authoritative (numbers drift). Note: *"into a file" ≠ "into the directory"* — spell out which.
- **Who edits what** — large, structural, or multi-section edits and new files: **Claude builds and hands back the finished file(s)** in `outputs/`; Daniel deploys. Small surgical edits: handed over in the FIND/REPLACE format above (or applied by Claude if Daniel prefers). When unsure which bucket an edit falls in, Claude asks rather than assuming.
...
- Never give Daniel a vague edit instruction — for surgical edits, hand him a **FIND block** (exact existing code) and a **REPLACE block** (the region after the change), per the *Surgical-edit handoff format*. Never "drop it in near X."
